...
const TodoPage = ({ userId, darkMode, setDarkMode, onLogout }) => {
  ...
  // Token'ı al (Yoksa boş string dönsün)
  const token = localStorage.getItem("token") || "";

  // Helper: Yetkili İstek Başlıkları
  ...
  // --- DOSYA YÜKLEME (ÖZEL HEADER AYARI) ---
  const handleUploadFile = async (todoId) => {
    const { value: file } = await Swal.fire({
      title: 'Dosya Seçin',
      input: 'file',
      inputAttributes: { 'accept': 'image/*, .pdf, .docx, .txt' },
    ...
    if (file) {
      ...
      fetch(`${API_URL}/upload/${todoId}`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }, // Content-Type OTOMATİK OLMALI (FormData)
        body: formData 
      })
      .then(async res => {
  ...
  const fetchCategories = () => {
    fetch(`${CAT_API_URL}?userId=${userId}`, { headers: getAuthHeaders() })
        .then(res => res.json())
        .then(data => {
            setCategories(data);
            if(data.length > 0 && newCategory === "") setNewCategory(data[0].name);
        });
  };

  const handleCategorySettings = async () => {
    const { value: action } = await Swal.fire({
        title: 'Kategori Yönetimi', input: 'radio', inputOptions: { 'add': '➕ Ekle', 'delete': '🗑️ Sil' },
        inputValidator: (v) => !v && 'Seçim yapmalısınız!', showCancelButton: true, confirmButtonText: 'Devam',
        background: darkMode ? '#333' : '#fff', color: darkMode ? '#fff' : '#000'
    });

    if (action === 'add') {
        const { value: f } = await Swal.fire({
            title: 'Yeni Kategori', html: '<input id="swal-input1" class="swal2-input" placeholder="Ad"><input id="swal-input2" type="color" class="swal2-input" style="height:50px;width:100px;" value="#0d6efd">',
            focusConfirm: false, showCancelButton: true, confirmButtonText: 'Kaydet', background: darkMode ? '#333' : '#fff', color: darkMode ? '#fff' : '#000',
            preConfirm: () => [document.getElementById('swal-input1').value, document.getElementById('swal-input2').value]
        });
        if (f && f[0]) {
            fetch(CAT_API_URL, { 
                method: "POST", headers: getAuthHeaders(), 
                body: JSON.stringify({ name: f[0], color: f[1], userId: parseInt(userId) }) 
            }).then(res => { if(res.ok) { toast.success("Eklendi!"); fetchCategories(); } });
        }
    } else if (action === 'delete') {
        const opts = {}; categories.forEach(c => opts[c.id] = c.name);
        const { value: cId } = await Swal.fire({ title: 'Silinecek kategori?', input: 'select', inputOptions: opts, showCancelButton: true, confirmButtonText: 'Sil', confirmButtonColor: '#d33', background: darkMode ? '#333' : '#fff', color: darkMode ? '#fff' : '#000' });
        if (cId) fetch(`${CAT_API_URL}/${cId}`, { method: "DELETE", headers: getAuthHeaders() }).then(res => { if(res.ok) { toast.info("Silindi."); fetchCategories(); } });
    }
  };

  // --- POMODORO ---
  useEffect(() => {
    if (isActive && timer > 0) { timerRef.current = setInterval(() => { setTimer((prev) => prev - 1); }, 1000); } 
    else if (timer === 0) { clearInterval(timerRef.current); setIsActive(false); if(mode === "work") { toast.success("Mola! ☕"); setMode("break"); setTimer(5*60); } else { toast.info("İş Başı! 🚀"); setMode("work"); setTimer(25*60); } }
    return () => clearInterval(timerRef.current);
  }, [isActive, timer, mode]);
  const toggleTimer = () => setIsActive(!isActive);
  const resetTimer = () => { setIsActive(false); setMode("work"); setTimer(25 * 60); };
  const formatTime = (s) => { const m = Math.floor(s / 60); const sc = s % 60; return `${m<10?'0':''}${m}:${sc<10?'0':''}${sc}`; };

  // --- API ---
  const fetchAPI = () => {
    setIsLoading(true);
    fetch(`${API_URL}?userId=${userId}`, { headers: getAuthHeaders() })
      .then(res => {
          if(res.status === 401) { onLogout(); throw new Error("Oturum süresi doldu"); }
          return res.json();
      })
  ...
  const handleAddSubTask = async (todoId) => {
    ...
    if (text) { fetch(`${API_URL}/subtask`, { method: "POST", headers: getAuthHeaders(), body: JSON.stringify({ title: text, isCompleted: false, todoItemId: todoId }) }).then(res => { if(res.ok) { toast.success("Eklendi"); fetchAPI(); } }); }
  };
  const handleToggleSubTask = (subId) => fetch(`${API_URL}/subtask/${subId}`, { method: "PUT", headers: getAuthHeaders() }).then(res => { if(res.ok) fetchAPI(); });
  const handleDeleteSubTask = (subId) => fetch(`${API_URL}/subtask/${subId}`, { method: "DELETE", headers: getAuthHeaders() }).then(res => { if(res.ok) { toast.info("Silindi"); fetchAPI(); } });

  // --- CRUD VE DND ---
  const handleOnDragEnd = (result) => {
    if (!result.destination) return;
    const items = Array.from(todos); const [reorderedItem] = items.splice(result.source.index, 1); items.splice(result.destination.index, 0, reorderedItem); setTodos(items);
    const sortedIds = items.map(t => t.id); fetch(`${API_URL}/reorder?userId=${userId}`, { method: "PUT", headers: getAuthHeaders(), body: JSON.stringify(sortedIds) });
  };
  const addItem = () => {
    if (!newItem) { toast.warn("Ad boş olamaz!"); return; }
    ...
    fetch(API_URL, { method: "POST", headers: getAuthHeaders(), body: JSON.stringify(taskToSend) }).then(res => res.json()).then(() => { toast.success("Eklendi!"); setNewItem(""); fetchAPI(); });
  };
  const deleteItem = (id) => { Swal.fire({ title: 'Silinsin mi?', icon: 'warning', showCancelButton: true, confirmButtonText: 'Evet', confirmButtonColor: '#d33', background: darkMode ? '#333' : '#fff', color: darkMode ? '#fff' : '#000' }).then((r) => { if (r.isConfirmed) fetch(`${API_URL}/${id}`, { method: "DELETE", headers: getAuthHeaders() }).then(res => { if(res.ok) { toast.info("Silindi"); fetchAPI(); } }); }); };
  const toggleComplete = (item) => updateRequest({ ...item, isCompleted: !item.isCompleted });
  const updateRequest = (taskObj) => fetch(`${API_URL}/${taskObj.id}`, { method: "PUT", headers: getAuthHeaders(), body: JSON.stringify(taskObj) }).then(res => { if(res.ok) fetchAPI(); });
  ...
  const getCategoryBadge = (catName) => { const cat = categories.find(c => c.name === catName); const color = cat ? cat.color : "#6c757d"; return <span className="badge ms-2 text-white" style={{ backgroundColor: color }}>{catName}</span>; };
  ...
  const calendarEvents = todos.map(todo => { const catObj = categories.find(c => c.name === todo.category); return { id: todo.id.toString(), title: todo.title, date: todo.dueDate ? todo.dueDate.split('T')[0] : new Date(), backgroundColor: catObj ? catObj.color : (todo.isCompleted ? '#198754' : '#0d6efd'), borderColor: 'transparent', allDay: true }; });
      ...
      <div className="card shadow-lg">
        ...
        <div className="card-body">
          ...
          <div className="row g-2 mb-4">
            ...
            <div className="col-md-3 d-flex gap-1"><select className="form-select" value={newCategory} onChange={(e)=>setNewCategory(e.target.value)}>{categories.map(cat => (<option key={cat.id} value={cat.name}>{cat.name}</option>))}</select><button className="btn btn-outline-secondary" onClick={handleCategorySettings} title="Kategori Yönetimi">⚙️</button></div>
          ...
          {isLoading ? (<div className="mt-3">{[...Array(6)].map((_, i) => (<div key={i} className="mb-2"><Skeleton height={60} borderRadius={8} baseColor={darkMode?"#2c3034":"#e0e0e0"} highlightColor={darkMode?"#444":"#f5f5f5"}/></div>))}</div>) : (
                        ...
                        <li ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} className={`list-group-item ${snapshot.isDragging?'bg-info bg-opacity-25 shadow':''}`} style={{...provided.draggableProps.style}}>
                          {editingId===t.id ? (
                            <div className="d-flex gap-2 flex-wrap">
                              ...
                              <select className="form-select" style={{width:"120px"}} value={editCategory} onChange={(e)=>setEditCategory(e.target.value)}>{categories.map(cat => <option key={cat.id} value={cat.name}>{cat.name}</option>)}</select>
                                ...
                                <div className="d-flex justify-content-between align-items-center">
                                  <div style={{flex:1, cursor:"pointer", display:"flex", alignItems:"center"}}>
                                     ...
                                     <div onClick={()=>toggleComplete(t)} style={{flex:1}}>
                                        ...
                                        {getCategoryBadge(t.category)} {getPriorityBadge(t.priority)} {formatDateInfo(t.dueDate, t.isCompleted)}
                                     </div>
                                  </div>
                                  <div className="d-flex align-items-center gap-1">
                                    {t.attachmentPath ? (<a href={`${BASE_URL}/${t.attachmentPath}`} target="_blank" rel="noopener noreferrer" className="btn btn-sm btn-outline-info me-2 rounded-circle p-0 d-flex align-items-center justify-content-center" style={{width:"32px", height:"32px"}} title="Dosya"><span style={{ fontSize: "16px", lineHeight: 0 }}>👁️</span></a>) : (<button className="btn btn-sm btn-outline-secondary me-2 rounded-circle p-0 d-flex align-items-center justify-content-center" style={{width:"32px", height:"32px"}} onClick={() => handleUploadFile(t.id)} title="Dosya Ekle"><span style={{ fontSize: "16px", lineHeight: 0 }}>📎</span></button>)}
